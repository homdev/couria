...
const Create = () => {
    ...
    const [selectedType, setSelectedType] = useState('Cours théorique & Image');
    const [paidMember, setPaidMember] = useState(false);
    const [lableText, setLableText] = useState('Pour les membres gratuits, le nombre de sous-thèmes est limité à 5');
    ...
    useEffect(() => {
        ...
        if (sessionStorage.getItem('type') !== 'free') {
            setPaidMember(true);
            setLableText('Sélectionnez le nombre de sous-thèmes')
        }
    ...
    let addFormFields = () => {
        if (formValues.length < maxSubtopics) {
            ...
            showToast('Vous pouvez ajouter jusqu\'à 5 sous-thèmes uniquement');
        }
    }
    ...
    const showPaidToast = async () => {
        if (!paidMember) {
            toast("Pour les membres pro uniquement", {
            ...
            <div className='dark:bg-black flex-1'>
                ...
                <div className='flex-1 flex items-center justify-center py-10'>
                    ...
                    <form onSubmit={handleSubmit} className="max-w-sm m-auto py-4 no-scrollbar">
                        <p className='text-center font-black text-4xl text-black dark:text-white'>Générer un cours</p>
                        <p className='text-center font-normal text-black py-4 dark:text-white'>Tapez le sujet sur lequel vous souhaitez générer un cours.<br />
                        En outre, vous pouvez saisir une liste de sous-thèmes, qui correspondent aux détails que vous souhaitez apprendre.</p>
                        <div className='py-6'>
                            <div className='mb-6'>
                            ...
                            <fieldset className="flex max-w-md flex-col gap-4 mt-2">
                                <div className="flex items-center gap-2 px-2 h-11 focus:ring-black focus:border-black border border-black font-normal bg-white rounded-none w-full dark:bg-black dark:border-white dark:text-white">
                                    ...
                                    <Label className='text-black dark:text-white font-bold' htmlFor="textcourse">Cours théorique & Image</Label>
                                </div>
                                <div onClick={() => showPaidToast()} className="flex items-center gap-2 px-2 h-11 focus:ring-black focus:border-black border border-black font-normal bg-white rounded-none w-full dark:bg-black dark:border-white dark:text-white mb-6">
                                    ...
                                    <Label className='text-black dark:text-white font-bold' htmlFor="videocourse">Cours vidéo et Théorie</Label>
                                </div>
                            </fieldset>

                            <Button isProcessing={processing} processingSpinner={<AiOutlineLoading className="h-6 w-6 animate-spin" />} className='items-center justify-center text-center dark:bg-white dark:text-black bg-black text-white font-bold rounded-none w-full enabled:hover:bg-black enabled:focus:bg-black enabled:focus:ring-transparent dark:enabled:hover:bg-white dark:enabled:focus:bg-white dark:enabled:focus:ring-transparent' type="submit">Envoyer</Button>
                        </div>
